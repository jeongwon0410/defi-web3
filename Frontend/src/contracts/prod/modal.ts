...
import ATOKEN_ABI from "./abi/AToken_ABI";
import {
  allDecimals,
  AssetTitle,
  lendingPoolAddr,
    ...
    switch (type) {
      ...
      case "withdraw":
        writeContract({
          ...
          functionName: "redeem",
          args: [BigInt(amount.multipliedBy(decimals).toFixed(0))],
        });
        break;
      case "borrow":
        writeContract({
...
export const useSupplyModalProd = (title: AssetTitle, close: () => void) => {
  const {
    approve,
    action: supply,
    amount,
...
export const useWithdrawModalProd = (
...
export const useBorrowModalProd = (
  assetTitle: AssetTitle,
  close: () => void,
) => {
  const {
  ...
  const balance =
    _balance !== undefined
      ? BN(_balance)?.dividedBy(decimals)
      : undefined;
  const borrowable =
    price &&
    BN(accountData?.[2])
...
export const useRepayModalProd = (title: AssetTitle, close: () => void) => {
  const {
    approve,
    action: repay,
    amount,
  ...
  const balance = BN(_balance)?.dividedBy(allDecimals[title]);

  const { data } = useUserReserveData(title);
  const decimals = allDecimals[title];
  const myDebt = BN(data?.[1])?.dividedBy(decimals);
...
export const useHealthFactorProd = (
  type: "borrow" | "repay",
  amount: BigNumber,
  title: AssetTitle,
) => {
  ...
  const totalSupply = BigNumber(data[1].toString());
  const totalBorrow = BigNumber(data[2].toString());
  const liquidationThreshold = BigNumber(data[5].toString());
