...
async function callContract(response) {
  try {
    ...
    const SimpleContract = await ethers.getContractFactory("SupraPriceOracle");

    // Connect to the deployed contract
    const contractAddress = "0x05b2B2fC77aBCa0baDa6A60723aD8dDCE9Fee075"; // Replace with your deployed contract address
    const contract = await SimpleContract.attach(contractAddress);

    // Set a new message in the contract
    let price = await contract.GetPairPrice(response.proof_bytes, 211);

    // Retrieve the updated message
    console.log("Updated Message:", price);
  } catch (error) {
    console.error(error);
    process.exit(1);
  }
