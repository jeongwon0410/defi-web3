...
export default function SupplyTable({ allMarket }: Props) {
  ...
  const checkSupplyRatio = async () => {
    DAIMySupplyRatio().then((item) => setDaiSupplyRatio(item ?? "0%"));
    USDTMySupplyRatio().then((item) => setUsdtSupplyRatio(item ?? "0%"));
    USDCMySupplyRatio().then((item) => setUsdcSupplyRatio(item ?? "0%"));
    WBTCMySupplyRatio().then((item) => setWbtcSupplyRatio(item ?? "0%"));
    LINKMySupplyRatio().then((item) => setLinkSupplyRatio(item ?? "0%"));
    AAVEMySupplyRatio().then((item) => setAaveSupplyRatio(item ?? "0%"));
    EURSMySupplyRatio().then((item) => setEursSupplyRatio(item ?? "0%"));
    WETHMySupplyRatio().then((item) => setWethSupplyRatio(item ?? "0%"));
  };
